...
public class RTSController : MonoBehaviour
{
    ...
    void GiveMoveCommand()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hitUnit, Mathf.Infinity, unitLayer))
        {
            UnitTeam clickedTeam = hitUnit.collider.GetComponent<UnitTeam>();
            
            if (clickedTeam != null)
            {
                bool attackCommandGiven = false;

                foreach (Unit unit in selectedUnits)
                {
                    if (unit == null) continue;

                    UnitTeam myTeam = unit.GetComponent<UnitTeam>();
                    
                    
                    if (myTeam != null && myTeam.teamID != clickedTeam.teamID)
                    {
                        TankCombat combat = unit.GetComponent<TankCombat>();
                        if (combat != null) combat.SetManualTarget(hitUnit.collider.transform);
                        
                        unit.MoveTo(unit.transform.position); 
                        attackCommandGiven = true;
                    }
                }

                
                if (attackCommandGiven) return; 
            }
        }

       
        if (Physics.Raycast(ray, out RaycastHit hitGround, Mathf.Infinity, groundLayer))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                TankCombat combat = selectedUnits[i].GetComponent<TankCombat>();
                if (combat != null) combat.ClearManualTarget();

                float xOffset = (i % cols - (cols - 1) / 2f) * spacing;
                float zOffset = (i / cols - (rows - 1) / 2f) * spacing;

                selectedUnits[i].MoveTo(hitGround.point + new Vector3(xOffset, 0, zOffset));
            }
        }
    }
